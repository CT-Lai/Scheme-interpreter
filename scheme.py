...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(procedure)# if not procedure it will raise. If it is, do nothing
...
def eval_all(expressions, env):
    ...
    while True:
        if expressions.second is nil:
            lastexp = expressions.first
            break
        scheme_eval(expressions.first, env)
        expressions = expressions.second
    return scheme_eval(lastexp, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("the number of variable and values not match")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        value = scheme_eval(Pair("lambda", Pair(target.second, expressions.second)),
                             env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    # normal case
    first, rest = scheme_eval(expressions.first, env), expressions.second
    if rest is nil :
        return first
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    def extract_formals_and_vals(bindings, formals=nil, vals=nil):
        if bindings is nil:
            return formals, vals
        pair = bindings.first
        check_form(pair, 2, 2)
        ...
        return extract_formals_and_vals(bindings.second, formals, vals) 
        
    formals, vals = extract_formals_and_vals(bindings)
    check_formals(formals)
